...
def train(cfg, local_rank, distributed, logger):
    ...
    train_data_loader = make_data_loader(
        ...
        # start_iter=arguments["iteration"], # 相当于把训练集当测试集中 这里就不用加起始的迭代数了
    )
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader):
        ...
        with torch.no_grad():
            relation_logits = model(images, targets) # 得到relation_logits, x*51
        ...
        for t, logits in zip(targets, relation_logits):
            ...
            cur_data = modify_logits(cur_data, logits) # 记录下每一张图片的谓语logit,转成cpu
            img_path = cur_data['img_path']
            if img_path in dic: # 如果有重复的图片路径了 说明第一遍对数据集的迭代已经结束 结束for
                end = True
                break
            else:
    ...
    print(len(dic))
    import json
    json.dump(dic, open("tmp/"+str(local_rank)+".json", "w")) # dict: 图片路径
    pickle.dump(to_save, open("tmp/"+str(local_rank)+".pk", "wb")) # to_save：每张图片的谓语logit
