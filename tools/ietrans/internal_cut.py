...
topk_percent = 0.7 # float(sys.argv[1])，选logit降序排列的前70%（logit大的）
path = "../output/relation_baseline/em_E.pk"

vocab = json.load(open("/root/autodl-nas/IETrans/datasets/vg/VG-SGG-dicts-with-attri.json", "r"))
...
def vis_triplet(triplet):
    logit = rel_dic[triplet[0]][(triplet[1], triplet[2])] # 得到这个三元组的logit(加和)
    scores, idxs = logit.sort(descending=True) # 按照logit降序排序
    prds = [idx2pred.get(int(i), "none") for i in idxs] # 50类谓语
...
def collect_all_parent_data(query_parents, query_pair, son_rel_idx):
    ...
    """
    sub_lb, obj_lb = lb2idx[query_pair[0]], lb2idx[query_pair[1]] # 类别id
...
importance_dic = {} # 按照三元组样本的数目多少，计算每种三元组的重要程度
...
# use vis_triplet
for triplet in tqdm(all_triplets): # 三元组的种类：29283种 pole-holding-wire→pole-in-wire
    # IPython.embed()
    # triplet: (r, sub, obj)
    r = triplet[0]
    # prds: [(rel, score)]
    prds = vis_triplet(triplet) # 比如说on-logo-shirt这种三元组，prds是维度为50的logit(由求和得来)
    # find parents
