...
def train(cfg, local_rank, distributed, logger):
    ...
    train_data_loader = make_data_loader(
        ...
        # start_iter=arguments["iteration"], # 暂时不用这个
    )
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader):
        ...
        for t, logits in zip(targets, relation_logits):
            ...
            cur_data = modify_logits(cur_data, logits) # 多了背景的logit，同时计算了possbile_rel的logit
...
def modify_logits(cur_data, rel_logits):
    ...
    rst_rel_logits = []
    rst_possible_rels = []
    for i, (rels, logits) in enumerate(zip(possible_rels, rel_logits)): # 前一个是给的 后一个是算的51
        rels = rels if type(rels) is list else [rels]
        rels = rels + [0] # 谓语为bg的logit
        rst_rel_logits.append(logits[rels].cpu().numpy())
        rst_possible_rels.append(rels)
    cur_data['rel_logits'] = rst_rel_logits # 在原来possible_rel基础上多一个bg的logit
    cur_data['possible_rels'] = rst_possible_rels # 在原来基础上多一个0
    return cur_data
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="/home/ubuntu/usefile/userfile/zrn/IETrans/configs/wsup-50.yaml",
        metavar="FILE",
    ...
    if dist.is_available(): # and dist.get_rank() == 0:
        ...
        for d in l:
            dic[d['img_path']] = d
        # rst_l = [dic[k] for k in file_names] # 改 原
        rst_l = [dic[k.split('zrn/')[-1]] for k in file_names]
