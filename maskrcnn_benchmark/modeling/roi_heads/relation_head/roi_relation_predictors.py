...
class CausalAnalysisPredictor(nn.Module):
    def __init__(self, config, in_channels, use_gt):
        ...
        if self.use_vtranse:
            ...
            self.ctx_compress = nn.Linear(self.pooling_dim, self.num_rel_cls)
        self.vis_compress = nn.Linear(self.pooling_dim // 4, self.num_rel_cls) # pair_level context 1024
        ...
        # ------------------TransH-------------------
        self.h_proj = nn.Sequential(*[nn.Linear(self.hidden_dim, self.hidden_dim),
                                    nn.ReLU(inplace=True),]) # 512→512
        self.t_proj = nn.Sequential(*[nn.Linear(self.hidden_dim, self.hidden_dim),
                                    nn.ReLU(inplace=True),])
        layer_init(self.h_proj[0], xavier=True)
        layer_init(self.t_proj[0], xavier=True)
        self.transh = True
        self.post_transh = nn.Sequential(*[nn.Linear(self.hidden_dim, 4096),
                                           nn.ReLU(inplace=True),]) # 512→4096
        layer_init(self.post_transh[0], xavier=True)

        # -----------------Semantic Oriented---------------
        self.use_gt = use_gt

    def pair_feature_generate(self, roi_features, proposals, rel_pair_idxs, num_objs, obj_boxs, global_features, logger,
                              ctx_average=False, rel_labels=None):
        # encode context infomation and pair-level context as union_feature
        obj_dists, obj_preds, edge_ctx, binary_preds, union_feature = self.context_layer(roi_features, proposals, rel_pair_idxs, global_features,
                                                                          logger, ctx_average=ctx_average, use_gt=self.use_gt, rel_labels=rel_labels)
        ...
        if self.transh:
            head_rep = head_rep - self.h_proj(head_rep)  # TransH, project edge_ctx to hyperplanes
            tail_rep = head_rep - self.t_proj(tail_rep)
        # split
        head_reps = head_rep.split(num_objs, dim=0)
        tail_reps = tail_rep.split(num_objs, dim=0)
        obj_preds = obj_preds.split(num_objs, dim=0)
        ...
        if self.transh:
            post_ctx_rep = self.post_transh(ctx_rep)
        else:
            post_ctx_rep = self.post_cat(ctx_rep)

        return post_ctx_rep, pair_pred, pair_bbox, pair_obj_probs, binary_preds, obj_dist_prob, edge_rep, obj_dist_list, union_feature

    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, global_features, logger=None):
        """
        Returns:
            obj_dists (list[Tensor]): logits of object label distribution
            rel_dists (list[Tensor])
        ...
        post_ctx_rep, pair_pred, pair_bbox, pair_obj_probs, binary_preds, obj_dist_prob, edge_rep, obj_dist_list, union_features = self.pair_feature_generate( # _:union_features
            roi_features, proposals, rel_pair_idxs, num_objs, obj_boxs, global_features, logger, rel_labels=rel_labels)
...
def make_roi_relation_predictor(cfg, in_channels, use_gt):
    func = registry.ROI_RELATION_PREDICTOR[cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR]
    return func(cfg, in_channels, use_gt)
