...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        # Semantic Oriented Learning -- two predictors
        self.predictor = make_roi_relation_predictor(cfg, feat_dim, use_gt = False)
        self.gto_predictor = make_roi_relation_predictor(cfg, feat_dim, use_gt = True)
        ...
        # parameters
        self.use_union_box = self.cfg.MODEL.ROI_RELATION_HEAD.PREDICT_USE_VISION # False
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        global_features = self.box_feature_extractor.forward_global_feature(features, proposals)
        ...
        if self.training:
           refine_logits, relation_logits, add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, global_features, logger)
           # Semantic-Oriented Predicted Distribution
           _, gto_relation_logits, _ = self.gto_predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, global_features, logger)
        else:
           refine_logits, relation_logits, add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, global_features, logger)
        ...
        if self.weak_sup and self.training:
            ...
            gto_relation_logits = cat(gto_relation_logits, dim=0)
            loss_kl = 30 * F.kl_div(relation_logits.softmax(dim=-1).log(), gto_relation_logits.softmax(dim=-1), reduction='mean') # KL loss
        else:
        ...
        if self.cfg.MODEL.ATTRIBUTE_ON and isinstance(loss_refine, (list, tuple)):
            ...
            output_losses = dict(loss_rel=loss_relation, loss_refine_obj=loss_refine, loss_kl=loss_kl)
