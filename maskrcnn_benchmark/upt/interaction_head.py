...
class InteractionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.coop_layer_dis = ModifiedEncoder(
            ...
            num_layers=1,
            return_weights=True, # Attention Distraction Layer
        )
        self.coop_layer_int = ModifiedEncoder(
            hidden_size=hidden_state_size,
            representation_size=representation_size,
            num_layers=1,
            return_weights=True  # Attention Integration Layer
        )
        ...
        self.pred_embed = torch.load('pred_embedding.pt') # path of predicate embedding
        self.pairwise_embed = nn.Linear(hidden_state_size*2 + 200, hidden_state_size * 2)
        layer_init(self.pairwise_embed, xavier=True)


    def forward(self, features, obj_ctx, proposals, global_features, rel_pair_idxs, use_gt, rel_labels):
        """
        Parameters:
        -----------
        ...
        if rel_labels!= None: # training
            for proposal, obj_ctx, global_feature, rel_pair_id, rel_label in zip(proposals, obj_ctxs, global_features, rel_pair_idxs, rel_labels):
                box = proposal.bbox
                ...
                box_pair_spatial_reshaped_all = box_pair_spatial_all.reshape(n, n, -1) # all the detected entities
                # Attention Distraction Layer
                unary_tokens_dis, unary_attn_dis = self.coop_layer_dis(obj_ctx, box_pair_spatial_reshaped_all)

                x_keep, y_keep = rel_pair_id[:,0], rel_pair_id[:,1] # foreground entities
                ...
                # Semantic Oriented Message Passing
                if use_gt:
                    assert len(rel_label) > 0
                    gt_label_id = torch.argmax(rel_label, dim=-1)
                    gt_embed = self.pred_embed[gt_label_id].to(device)

                    pairwise_tokens = torch.cat([
                        self.mbf( self.reduce_channel(torch.cat([unary_tokens[x_keep], unary_tokens[y_keep]], 1)),
                            box_pair_spatial_reshaped[x_keep, y_keep]),
                        self.mbf_g( global_feature[None, :],
                            box_pair_spatial_reshaped[x_keep, y_keep]),
                        gt_embed       # Teacher stage with predicate embedding
                    ], dim=1)
                    pairwise_tokens = self.pairwise_embed(pairwise_tokens)
                else:
                    pairwise_tokens = torch.cat([
                        self.mbf(
